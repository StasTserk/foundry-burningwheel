...
export function handleLearningRoll({
    ...
    dataPreset,
    onRollCallback,
}: LearningRollOptions): Promise<unknown> | Application {
    if (skill.data.data.root2) {
        return new Dialog({
            title: 'Pick Root Stat',
            content:
                '<p>The skill being learned is derived from two roots. Pick one to use for the roll.</p>',
            buttons: {
                root1: {
                ...
                root2: {
                    ...
                    callback: () => {
                        return buildLearningDialog({
                            actor,
                            skill,
                            statName: skill.data.data.root2,
                            extraInfo,
                            dataPreset,
                            onRollCallback,
                        });
    ...
    switch (difficultyGroup) {
        ...
        case 'Routine/Difficult':
            ...
            const dialog = new Dialog({
                ...
                content:
                    "<p>This test can count as routine of difficult for the purposes of advancement</p><p>Pick which option you'd prefer.</p>",
                buttons: {
                    skill: {
