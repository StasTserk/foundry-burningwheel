...
export function handleWeaponRollEvent({
    ...
    sheet,
    dataPreset,
}: EventHandlerOptions): Promise<unknown> | Application {
    ...
    const weapon = sheet.actor.getOwnedItem(weaponId) as
        | MeleeWeapon
        | RangedWeapon;
    if (!weapon) {
...
export function handleWeaponRoll({
    actor,
    weapon,
    attackIndex,
    skill,
    dataPreset,
}: WeaponRollOptions): Promise<unknown> | Application {
    ...
    if (quality === 'superior') {
        weaponPreset = {
            diceModifiers: [
                { dice: 1, label: 'Superior Quality', optional: false },
            ],
        };
    } else if (quality === 'poor') {
        weaponPreset = {
            obModifiers: [
                { obstacle: 1, label: 'Poor Quality', optional: false },
            ],
        };
    }
    ...
    if (weapon.type === 'melee weapon') {
        weaponExtraData = (weapon as MeleeWeapon).getWeaponMessageData(
            attackIndex || 0
        );
    } else {
