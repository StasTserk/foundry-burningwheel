    ...
    html.find('div.skills-grid').each((_, e) => {
        ...
        if (skillId) {
            ...
            if (skillData) {
                ...
                (skillData.data as SkillData).shade = costToString(
        ...
        if (traitId) {
            ...
            traits.push({
                data: {
                    ...
                    text:
                        'Unknown trait created during character burning. Update data accordingly. If the trait adds a reputation or affiliation, those must be added in manually.',
                } as TraitData,
                type: 'trait',
                name: traitName,
                img:
                    constants.defaultImages[
...
export function extractReputationData(
    html: JQuery<HTMLElement>
): Partial<ReputationDataRoot | AffiliationDataRoot>[] {
