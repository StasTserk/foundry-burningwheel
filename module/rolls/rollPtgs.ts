...
export async function handleShrugRollEvent({
    target,
    sheet,
    dataPreset,
...
export async function handleGritRollEvent({
    target,
    sheet,
    dataPreset,
}: EventHandlerOptions): Promise<unknown> {
        ...
        buttons.withPersona = {
            ...
            callback: async (_: JQuery) => {
                ...
                updateData['data.health.persona'] =
                    (actor.data.data.health.persona || 0) + 1;
        ...
        buttons.withFate = {
            ...
            callback: async (_: JQuery) => {
                ...
                updateData['data.health.fate'] =
                    (actor.data.data.health.fate || 0) + 1;
                return actor.update(updateData);
            },
        };
    }
...
async function ptgsRollCallback(
    ...
    const callons: RerollData[] = sheet.actor.getCallons('health').map((s) => {
        return {
            label: s,
            ptgsAction: shrugging ? 'shrugging' : 'gritting',
            ...(buildRerollData({
                actor: sheet.actor,
                roll,
                accessor: 'data.health',
