...
export class MeleeWeapon extends BWItem {
    ...
    getWeaponMessageData(attackIndex: number): string {
        ...
        element.appendChild(
            helpers.DivOfText(
                `${this.name} ${this.data.data.attacks[attackIndex].attackName}`,
        ...
        element.appendChild(
            helpers.DivOfText(
                translateWoundValue(
                    ...
                    this.data.data.attacks[attackIndex].incidental || 1
                )
            )
        );
        element.appendChild(
            helpers.DivOfText(
                translateWoundValue(
                    this.data.data.shade,
                    this.data.data.attacks[attackIndex].mark || 1
                )
            )
        );
        element.appendChild(
            helpers.DivOfText(
                translateWoundValue(
                    this.data.data.shade,
                    this.data.data.attacks[attackIndex].superb || 1
                )
            )
        );
        element.appendChild(
            helpers.DivOfText(this.data.data.attacks[attackIndex].add)
        );
        element.appendChild(
            helpers.DivOfText(this.data.data.attacks[attackIndex].vsArmor)
        );
        element.appendChild(
            helpers.DivOfText(
                this.data.data.attacks[attackIndex].weaponLength.titleCase()
            )
        );
