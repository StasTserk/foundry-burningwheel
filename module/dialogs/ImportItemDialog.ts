...
export async function addNewItem(
    ...
    const loadExistingCallback = async (
        ...
        const sourceList = ['World'].concat(
            Array.from(game.packs.values()) // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        ...
                        callback: async (dialogHtml: JQuery) => {
                            const newItems = dialogHtml
                                ...
                                .map((_, element: HTMLInputElement) => {
                                    const itemRoot = (items.find(
                                        (s: BWItem) => s._id === element.value
                                    ) as BWItem).data;
                                    Object.assign(
                                        itemRoot.data,
                                        options.forcedData
        ...
        dialog.sources =
            actor.getFlag(constants.systemName, 'compendia') ||
            ([] as string[]);
        dialog.render(true);
    };
