...
export async function handleNpcStatRollEvent({
    ...
    const statName = (target.dataset.rollableName ||
        'Unknown Stat') as NpcStatName;
    ...
    const data = mergeDialogData<NpcStatDialogData>(
        {
            ...
            arthaDice: 0,
            woundDice:
                ['circles', 'resources', 'health'].indexOf(statName) === -1
                    ? actor.data.data.ptgs.woundDice
                    : 0,
            obPenalty:
                ['circles', 'resources', 'health'].indexOf(statName) === -1
                    ...
                    : 0,
            circlesBonus:
                statName === 'circles' ? actor.data.circlesBonus : undefined,
            circlesMalus:
                statName === 'circles' ? actor.data.circlesMalus : undefined,
