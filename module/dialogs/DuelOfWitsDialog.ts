...
export class DuelOfWitsDialog extends ExtendedTestDialog<DuelOfWitsData> {
    ...
    private _handleRoll(e: JQuery.ClickEvent) {
        ...
        const actor = game.actors.entities.find(
            (a) => a._id === target.dataset.actorId
        ) as BWActor;
        const skill = actor?.getOwnedItem(target.dataset.skillId || '') as
            | Skill
            | undefined;
    ...
    getData(): DuelOfWitsData {
        ...
        data.actor1 = actors.find((a) => a._id === data.side1ActorId) as
            | BWActor
            | undefined;
        data.actor1Skills = (data.actor1?.data.socialSkills || []).map(
            (s: SkillDataRoot & { _id: string }) => {
                return { id: s._id, label: s.name };
            }
        );
        data.actor2 = actors.find((a) => a._id === data.side2ActorId) as
            | BWActor
            | undefined;
        data.actor2Skills = (data.actor2?.data.socialSkills || []).map(
            (s: SkillDataRoot & { _id: string }) => {
                return { id: s._id, label: s.name };
            }
        );
