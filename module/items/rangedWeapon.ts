...
export class RangedWeapon extends BWItem<RangedWeaponData> {
    ...
    async getWeaponMessageData(): Promise<string> {
        ...
        const roll = (await new Roll('1d6').roll({ async: true })).dice[0]
        ...
        element.appendChild(
            helpers.DivOfText(
                `${this.system.incidentalLabel}/${this.system.markLabel}/${this.system.superbLabel}`
            )
        );
