...
import { ChatFixture } from './parts/ChatFixture';
...
import { RollFixture } from './parts/RollDialog';
...
const extender: Parameters<typeof testLoggedIn.extend<BwFixture>>[0] = {
    ...
    rollDialog: async ({ page }, use) => await use(new RollFixture(page)),
    ...
    chat: async ({ page, gamePage }, use) =>
        await use(new ChatFixture(page, gamePage)),
