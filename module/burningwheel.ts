...
function registerHelpers() {
    ...
    Handlebars.registerHelper('add', (...args): number => {
        return (Array.prototype.slice.call(args, 0, -1) as (
            | string
            | number
...
Hooks.on(
    ...
    (actor: BWActor, item: ItemData, _options: any, userId: string) => {
        if (actor.data.type !== 'setting') {
            actor.processNewItem(item, userId);
        }
    }
);
Hooks.on('hotbarDrop', (_bar, data, slot) =>
    CreateBurningWheelMacro(data, slot)
);
