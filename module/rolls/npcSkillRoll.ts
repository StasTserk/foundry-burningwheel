...
export async function handleNpcWeaponRollEvent({
    target,
    sheet,
    dataPreset,
    ...
    if (!skillId) {
        return notifyError(
            'No Weapon Skill',
            'No Weapon Skill Chosen. Set the sheet into edit mode and pick a Martial skill to use with this weapon.'
        );
    }
    const skill = sheet.actor.getOwnedItem(skillId) as Skill;
    const weapon = sheet.actor.getOwnedItem(itemId) as
        | MeleeWeapon
        | RangedWeapon;
...
export async function handleNpcWeaponRoll({
    actor,
    weapon,
    skill,
    attackIndex,
    dataPreset,
}: NpcRollOptions): Promise<unknown> {
    if (!weapon) {
        return notifyError(
            'Missing Weapon',
...
export async function handleNpcSpellRollEvent({
    target,
    sheet,
    dataPreset,
}: NpcEventHandlerOptions): Promise<unknown> {
    ...
    if (!skillId) {
        return notifyError(
            'No Sorcerous Skill',
            'No Casting Skill Chosen. Set the sheet into edit mode and pick a Sorcerous skill to use with this weapon.'
        );
    }
    const skill = sheet.actor.getOwnedItem(skillId) as Skill;
...
export async function handleNpcSpellRoll({
    actor,
    spell,
    skill,
    dataPreset,
    ...
    if (!spell) {
        return notifyError(
            'Missing Spell',
            'The spell that is being cast appears to be missing from the character sheet.'
        );
    }
    const obstacle = spell.data.data.variableObstacle
        ? 3
        : spell.data.data.obstacle;
    if (dataPreset) {
...
export async function handleNpcSkillRollEvent({
    target,
    sheet,
    extraInfo,
    dataPreset,
...
export async function handleNpcSkillRoll({
    actor,
    skill,
    extraInfo,
    dataPreset,
    ...
    if (actor.data.data.persona) {
        dataPreset.personaOptions = Array.from(
            Array(Math.min(actor.data.data.persona, 3)).keys()
        );
    }
    ...
    if (skill.data.data.learning) {
        ...
        if (skill.data.data.root2) {
            ...
            return new Dialog({
                ...
                content:
                    '<p>The listed skill uses one of two possible roots. Pick one to roll.</p>',
                buttons: {
                    root1: {
                    ...
                    root2: {
                        ...
                        callback: () => {
                            const stat2 = getProperty(
                                actor.data,
                                `data.${skill.data.data.root2}`
                            ) as Ability;
                            rollData.dice = stat2.exp;
                            rollData.shade = stat2.shade;
                            rollData.open = stat2.open;
                            rollData.statName = skill.data.data
                                .root2 as NpcStatName;
...
async function skillRollCallback(
    ...
    const wildForkDie = await rollWildFork(
        rollData.wildForks,
        skill.data.data.shade
    );
    ...
    const isSuccessful =
        parseInt(roll.result) + wildForkBonus >= rollData.difficultyTotal;
