...
export async function handleSpellRoll({
    ...
    if (skill) {
        const obstacle = spell.system.variableObstacle
            ? 3
            : spell.system.obstacle;
        let practicalsPenalty = 0;
        const spellPreset: Partial<RollDialogData> = { difficulty: obstacle };
        if (spell.system.inPracticals) {
            practicalsPenalty =
                (spell.system.aptitude || 9) - spell.system.learningProgress ||
                0;
            spellPreset.obModifiers = [
                {
                    label: game.i18n.localize('BW.roll.inPracticals'),
                ...
                if (amount + 1 >= aptitude) {
                    return Dialog.confirm({
                        ...
                        content: `<p>${game.i18n.localize(
                            'BW.dialog.practicalsDoneText1'
                        )}</p><p>${game.i18n.localize(
                            'BW.dialog.practicalsDoneText2'
                        )}</p>`,
                        yes: () => {
                            spell.update(
                                {
