...
export class Spell extends BWItem {
    prepareData(): void {
        ...
        this.data.obstacleLabel = `${
            this.data.data.variableObstacle
                ...
                : this.data.data.obstacle
        }${this.data.data.upSpell ? '^' : ''}`;
        if (this.data.data.isWeapon && this.data.hasOwner && this.actor) {
            ...
            if (this.data.data.halfWill) {
                this.data.data.mark =
                    Math.floor(willScore / 2.0) +
                    this.data.data.willDamageBonus;
            } else {
                this.data.data.mark =
                    willScore + this.data.data.willDamageBonus;
            }
        ...
        if (this.data.hasOwner && this.actor) {
            this.data.data.aptitude =
                10 - this.actor.data.data.perception.exp ||
                1 +
    ...
    getSpellMessageData(): string {
        ...
        if (this.data.data.isWeapon) {
            ...
            element.appendChild(
                DivOfText(
                    'B ' + this.data.data.incidental,
                    roll < 3 ? 'highlight' : undefined
                )
            );
            element.appendChild(
                DivOfText(
                    'B ' + this.data.data.mark,
                    [3, 4].includes(roll) ? 'highlight' : undefined
                )
            );
            element.appendChild(
                DivOfText(
                    'B ' + this.data.data.superb,
                    roll > 4 ? 'highlight' : undefined
                )
            );
