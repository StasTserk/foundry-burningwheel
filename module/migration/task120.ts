...
export async function task120(): Promise<void> {
    const items: Skill[] = Array.from(game.items?.values() || []).filter(
        (i: BWItem) => i.type === 'skill'
    ) as Skill[];
    const updateInfo = {};
    ...
    for (const actor of actors) {
        for (const ownedItem of Array.from(actor.items.values()).filter(
            (i: BWItem) => i.type === 'skill'
        ) as Skill[]) {
    ...
    for (const pack of packs) {
        if (pack.documentName === 'Item') {
            ...
            for (const item of Array.from(packItems.values()).filter(
                (i: BWItem) => i.type === 'skill'
            ) as Skill[]) {
                const updateData = updateItem(item, updateInfo);
                if (Object.values(updateData).length) {
...
function updateItem(
    ...
    if (
        ...
        ['military', 'martial'].indexOf(item.system.skilltype) === -1
    ) {
