...
export class RangedWeapon extends BWItem {
    prepareData(): void {
        ...
        if (this.actor && this.data.data.usePower) {
            let baseDmg =
                this.actor.data.data.power.exp + this.data.data.powerBonus;
        ...
        this.data.data.markLabel =
            markRange - 1 === incidentalRange
                ? `${markRange}`
                : `${incidentalRange + 1}-${markRange}`;
        this.data.data.superbLabel =
            markRange === 5 ? `6` : `${markRange + 1}-6`;
    ...
    getWeaponMessageData(): string {
        ...
        element.appendChild(
            helpers.DivOfText(this.name, 'ims-title shade-black')
        );
        ...
        element.appendChild(
            helpers.DivOfText(
                helpers.translateWoundValue(
                    this.data.data.shade,
                    this.data.data.incidental
                ),
                incidental ? 'highlight' : undefined
            )
        );
        element.appendChild(
            helpers.DivOfText(
        ...
        element.appendChild(
            helpers.DivOfText(
                helpers.translateWoundValue(
                    this.data.data.shade,
                    this.data.data.superb
                ),
                !incidental && !mark ? 'highlight' : undefined
            )
        );
        element.appendChild(helpers.DivOfText(this.data.data.vsArmor));
        element.appendChild(
            helpers.DivOfText(
                `${this.data.data.incidentalLabel}/${this.data.data.markLabel}/${this.data.data.superbLabel}`
            )
        );
