...
export async function handleFightRoll({
    ...
    (dataPreset.offerSplitPool = true),
        ...
        (dataPreset.personaOptions = actor.data.data.persona
            ? Array.from(Array(Math.min(actor.data.data.persona, 3)).keys())
            : undefined);
    ...
    if (type === 'skill') {
        ...
        switch (item.type) {
            ...
            case 'ranged weapon':
                ...
                const weaponSkill = actor.getOwnedItem(
                    item.data.data.skillId
                ) as Skill;
            ...
            case 'spell':
                ...
                const skill = actor.getOwnedItem(
                    spell?.data.data.skillId
                ) as Skill;
                if (actor.data.type === 'character') {
