...
export async function handleTraitorReroll(
    ...
    if (isDeeds && actor.data.data.deeds == 0) {
        return helpers.notifyError(
            'No Deeds Available',
            'The character must have a deeds point available in order to reroll all traitors.'
        );
    }
            ...
            if (target.dataset.rerollType === 'stat') {
                if (successes <= obstacleTotal && success) {
                    ...
                    if (target.dataset.ptgsAction) {
                        ...
                        updateData[
                            `data.ptgs.${target.dataset.ptgsAction}`
                        ] = true;
                        actor.update(updateData);
                    }
                    if (
                        actor.data.successOnlyRolls.indexOf(
                            name.toLowerCase()
                        ) !== -1
                    ) {
                ...
                updateData[`${accessor}.deeds`] = isDeeds
                    ? parseInt(
                          getProperty(actor, `data.${accessor}.deeds`) || '0'
                      ) + 1
                    : undefined;
            } else if (target.dataset.rerollType === 'learning') {
                const learningTarget = target.dataset.learningTarget || 'skill';
                if (
                    actor.data.successOnlyRolls.includes(learningTarget) &&
                ...
                await skill.update(
                    { 'data.deeds': skill.data.data.deeds + 1 },
                    {}
                );
            }
        }
