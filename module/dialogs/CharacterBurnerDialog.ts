...
export class CharacterBurnerDialog extends Dialog {
    ...
    getData(): CharacterBurnerData {
        ...
        this._traits.forEach((s) => {
            const trait = {
                name: s.name,
                label: s.data.data.pointCost
                    ? `${s.name} - ${s.data.data.pointCost} Pts`
                    : s.name,
            };
        ...
        this._gear.forEach((g) => {
            const entry = {
                name: g.name,
                label: g.data.data.pointCost
                    ? `${g.name} - ${g.data.data.pointCost} Pts`
                    : g.name,
            };
            switch (g.type) {
                case 'melee weapon':
    ...
    activateListeners(html: JQuery): void {
        this._burnerListeners = [
            ...
            html
                .find(
                    "input[name='skillAdvances'], input[name='skillOpened'], input[name='skillTraining'], select[name='skillShade']"
                )
        ...
        if (data.type === 'Item' && data.id) {
            ...
            if (data.actorId) {
                if (data.pack) {
                    ...
                    const actor = (await (game.packs.find(
                        (p) => p.collection === data.pack
                    ) as Compendium).getEntity(data.actorId)) as Actor;
                    item = actor.getOwnedItem(data.id) as BWItem;
                } else {
                    item = (game.actors.find(
                        (a: BWActor) => a._id === data.actorId
                    ) as BWActor).getOwnedItem(data.id) as BWItem;
                }
            } else if (data.pack) {
                item = (await (game.packs.find(
                    (p) => p.collection === data.pack
                ) as Compendium).getEntity(data.id)) as BWItem;
            } else {
        ...
        switch (itemType) {
            case 'skill':
                ...
                if (
                    ...
                    if (
                        element.find("option[value='" + itemName + "']").length
                    ) {
    ...
    private _calculateSkillWorth(e: JQuery.ChangeEvent): void {
        ...
        parent
            ...
            .val(advances + shade + open + training + refund)
            .trigger('change');
    }
    ...
    _tryLoadTrait(e: JQuery.TriggeredEvent): void {
        ...
        if (!trait) {
            ...
            inputTarget
                ...
                .val(trait.data.data.traittype)
                .trigger('change');
            inputTarget.siblings("*[name='traitId']").val(trait._id);
            const cost =
                !trait.data.data.pointCost || isNaN(trait.data.data.pointCost)
                    ? 1
                    : trait.data.data.pointCost;
            inputTarget
                .siblings("*[name='traitCost']")
                .val(cost)
                .trigger('change');
    ...
    private _tryLoadSkill(e: JQuery.TriggeredEvent): void {
        ...
        if (!skill) {
            ...
            inputTarget
                ...
                .val(skill.data.data.root1)
                .trigger('change');
            inputTarget
                .siblings("*[name='skillRoot2']")
                .val(skill.data.data.root2)
                .trigger('change');
            inputTarget
                .siblings("*[name='skillTraining']")
                .prop('checked', skill.data.data.training);
    ...
    private _tryLoadProperty(e: JQuery.TriggeredEvent): void {
        ...
        if (!property) {
            ...
            inputTarget
                .siblings("*[name='propertyCost']")
                .val(property.data.data.pointCost || 0)
                .trigger('change');
        }
    }
    ...
    private _tryLoadGear(e: JQuery.TriggeredEvent): void {
        ...
        if (!gear) {
            ...
            inputTarget
                .siblings("*[name='itemCost']")
                .val((gear.data.data as HasPointCost).pointCost || 0)
                .trigger('change');
        }
    }
    ...
    private async _finishBurning(
        ...
        return Dialog.confirm({
            ...
            yes: async () => {
                ...
                return this._parent.createEmbeddedEntity(
                    'OwnedItem',
                    [
                        ...skillData,
                        ...traitData,
