...
export class Armor extends BWItem {
    prepareData(): void {
        ...
        this.data.data.helmDisplayClass = this.calculateDisplayClass(
            dice,
            this.data.data.damageHelm
        );
        this.data.data.torsoDisplayClass = this.calculateDisplayClass(
            dice + 1,
            this.data.data.damageTorso
        );
        this.data.data.leftArmDisplayClass = this.calculateDisplayClass(
            dice,
            this.data.data.damageLeftArm
        );
        this.data.data.rightArmDisplayClass = this.calculateDisplayClass(
            dice,
            this.data.data.damageRightArm
        );
        this.data.data.leftLegDisplayClass = this.calculateDisplayClass(
            dice,
            this.data.data.damageLeftLeg
        );
        this.data.data.rightLegDisplayClass = this.calculateDisplayClass(
            dice,
            this.data.data.damageRightLeg
        );
        this.data.data.shieldDisplayClass = this.calculateDisplayClass(
            dice,
            this.data.data.damageShield
        );
    }
    ...
    async assignDamage(roll: Roll, location: string): Promise<number> {
        ...
        const damage =
            parseInt(getProperty(this, `data.${locationAccessor}`)) || 0;
        const updateData = {};
        let newDamage = 0;
