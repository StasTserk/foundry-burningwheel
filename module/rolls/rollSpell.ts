...
export async function handleSpellRollEvent({
    target,
    sheet,
    dataPreset,
...
export async function handleSpellRoll({
    actor,
    spell,
    skill,
    dataPreset,
}: SpellRollOptions): Promise<unknown> {
    if (!spell) {
        return helpers.notifyError(
            'Missing Spell',
    ...
    if (skill) {
        ...
        if (spell.data.data.inPracticals) {
            practicalsPenalty =
                (spell.data.data.aptitude || 9) -
                    spell.data.data.learningProgress || 0;
            spellPreset.obModifiers = [
                {
                    label: 'In Practicals',
                    obstacle: practicalsPenalty,
                    optional: false,
                },
            ];
        }
        ...
        if (dataPreset) {
            ...
            dataPreset.obModifiers = (dataPreset.obModifiers || []).concat(
                ...(spellPreset.obModifiers || [])
            );
        } else {
            ...
            if (spell.data.data.inPracticals) {
                ...
                if (amount + 1 >= aptitude) {
                    return Dialog.confirm({
                        ...
                        content:
                            '<p>The spell practicals process is complete. Time for a second reading.</p><p>Set the spell as no longer in practicals?</p>',
                        yes: () => {
                            spell.update(
                                {
