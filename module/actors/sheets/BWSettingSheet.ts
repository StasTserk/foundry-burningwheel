...
export class BWSettingSheet extends ActorSheet {
    ...
    getData(): BWSettingData {
        ...
        data.lifepaths = (Array.from(this.actor.items.values()) as Lifepath[])
            .map((i) => i.data)
    ...
    async insertItemAtIndex(event: DragEvent, index: number): Promise<void> {
        ...
        ) as Lifepath[]).sort((a, b) => a.data.data.order - b.data.data.order);
        if (dragData.actorId === this.actor.id) {
            ...
            if (dragData.data) {
                ...
                itemData = (
                    await (game.packs.find(
                        (p) => p.collection === dragData.pack
                    ) as Compendium).getEntity(dragData.id || '')
                ).data as LifepathRootData;
            } else if (dragData.actorId) {
                itemData = game.actors
                    .find((a: BWSetting) => a._id === dragData.actorId)
                    .getOwnedItem(dragData.id).data as LifepathRootData;
            } else {
                itemData = game.items.find((i: BWItem) => i.id === dragData.id)
                    .data as LifepathRootData;
            }
