...
export class BWCharacter extends BWActor {
    ...
    prepareData(): void {
        ...
        updateTestsNeeded(
            this.data.data.forte,
            false,
            woundDice,
            this.data.data.forteTax
        );
        ...
        this.data.data.maxSustained =
            this.data.data.will.exp - (this.data.data.ptgs.woundDice || 0) - 1;
        this.data.data.maxObSustained =
            this.data.data.forte.exp -
        ...
        const unRoundedReflexes =
            (this.data.data.perception.exp +
                ...
                this.data.data.speed.exp) /
            3.0;
        this.data.data.reflexesExp =
            (this.data.data.settings.roundUpReflexes
                ? Math.ceil(unRoundedReflexes)
                : Math.floor(unRoundedReflexes)) -
            (this.data.data.ptgs.woundDice || 0);
        const shades = [
            this.data.data.perception.shade,
            this.data.data.agility.shade,
            this.data.data.speed.shade,
        ];
        this.data.data.reflexesShade = getWorstShadeString(
            getWorstShadeString(shades[0], shades[1]),
            shades[2]
        );
        if (this.data.data.reflexesShade === 'B') {
            this.data.data.reflexesExp += shades.filter(
                (s) => s !== 'B'
            ).length;
        ...
        this.data.data.mortalWoundShade = getWorstShadeString(
            this.data.data.power.shade,
            this.data.data.forte.shade
        );
        ...
        this.data.successOnlyRolls = (
            this.data.data.settings.onlySuccessesCount || ''
        )
            .split(',')
    ...
    private _calculatePtgs() {
        ...
        if (suCount >= 3) {
            ...
            !this.data.data.ptgs.shrugging &&
            suCount >= 1 &&
            !this.data.data.settings.ignoreSuperficialWounds
        ) {
        ...
        woundDice = Math.max(
            0,
            woundDice -
                (this.data.data.ptgs.woundRecovery1 || 0) -
                (this.data.data.ptgs.woundRecovery2 || 0) -
                (this.data.data.ptgs.woundRecovery3 || 0)
        );
    ...
    async addStatTest(
        ...
        const onlySuccessCounts =
            this.data.successOnlyRolls.indexOf(name) !== -1;
    ...
    public updateArthaForStat(
        ...
        updateData[`${accessor}.deeds`] = deeds
            ? (stat.deeds || 0) + 1
            : undefined;
        ...
        if (amount <= maxFundLoss) {
            ...
            if (resourcesTax === resourceExp) {
                ...
                new Dialog({
                    ...
                    content:
                        '<p>Tax has reduced your resources exponent to 0.</p><hr>',
                    buttons: {
                        reduce: {
